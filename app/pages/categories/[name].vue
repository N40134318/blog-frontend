...
const loadPosts = async () => {
  ...
  try {
    ...
    errorMessage.value = error?.message || '加载分类文章失败'
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="min-h-screen bg-gray-100">
    <div class="max-w-6xl mx-auto px-4 py-8">
      <div class="mb-8">
        <NuxtLink to="/categories" class="text-sm text-blue-600 hover:text-blue-700">
          ← 返回分类
        </NuxtLink>
        <h1 class="mt-3 text-3xl font-bold text-gray-900">分类：{{ categoryName }}</h1>
        <p class="mt-2 text-gray-600">该分类下的文章列表</p>
      </div>
